...
def test_celery_off_prunes_celery_module(cookies, run_manage):
...
def test_celery_on_keeps_celery_module(cookies, run_manage):
...
def test_sso_on_bakes_and_checks(cookies, run_manage):
...
def test_htmx_on_bakes_and_checks(cookies, run_manage):
...
def test_ninja_on_bakes_and_checks(cookies, run_manage):
...
def test_ninja_off_prunes_api(cookies, run_manage):
...
def test_redis_off_omits_cache(cookies, run_manage):
...
def test_s3_on_wires_storage_and_checks(cookies, run_manage):
...
def test_s3_off_omits_storage(cookies, run_manage):
